...
\begin{document}
    ...
    \section*{Properties of the Estimator: Monte Carlo Simulations}
        ...
        In our first set of simulations, we assume default values of $\rho = 0.5$, $\beta_1 = \beta_2 = 1, N = 3,000$, and $p=5$. These are standard values, and we feel that they best match the empirical application presented later in this paper. We then vary each factor while holding the others fixed, and perform 1,000 simulations of the DGP followed by an OLS regression on either the PCA value from the $p$ measurements of the true $z_i$, or on a single one of the measurements of $z_i$. The first two rows of each panel in Table \ref{sim_rho_3000_no_exp} show the results for different values of $\rho$.
