...
\begin{document}
    ...
    \section*{Application: Government Share of Healthcare Spending and Life Expectancy}
        ...
        Mini literature review on this question
        ...
        Hence, this setup, with a covariate in regression subject to measurement error, fits the situation described in the theory and simulations in the previous sections. In this case, we aim to to accurately reduce possible bias in the coefficient of the government's share of health spending by reducing bias in the coefficients for development indicators.
        ...
        In the left column in the table begin with an OLS regression. In the middle column we include the economic controls/covariates of GDP per capita (PPP), GNI per capita (PPP), Survey Mean Income/Consumption Per Capita, ILO GDP per person employed, and Net Foreign Assets Per Capita. In the rightmost column we instead use the first principal component combining these covariates.

        % Add: include just a single mismeasured covariate

        \input{../Output/Tables/LE_Health_Econ_Regressions.tex}

        The results clearly demonstrate that the principal components method achieves a noticably different coefficient on the government health share coefficient relative to multivariate OLS; in this case, a larger one.

        The univariate omitted variable bias formula yields that $\beta_{OLS} = \beta_{true} + \delta\gamma$ where $\delta$ is the effect of GDP on the government share and $\gamma$ is the effect of GDP on life expectancy, OLS naturally provides an overestimate. It is likely $\delta, \gamma > 0$, but mismeasurement leads to an attenuated $\gamma$

        NOTE- the math as worked out in the previous paragraph doesn't make sense.
