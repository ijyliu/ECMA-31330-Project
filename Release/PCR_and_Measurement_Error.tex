...
\begin{table}[!htbp] \centering
  ...
  \input{../Output/Tables/covariance_coefs_no_exp.tex}
    & \multicolumn{5}{c}{\textit{Mean Absolute Percentage Error}} \\
    \input{../Output/Tables/covariance_apes_no_exp.tex} \\[-4mm]
    \hline \\[-1.8ex]
...
\begin{table}[!htbp] \centering
  ...
  \hline \\[-1.8ex]
  & \multicolumn{5}{c}{\textit{Coefficient on Main Variable}} \\
  \input{../Output/Tables/covariance_coefs_exp.tex}
  & \multicolumn{5}{c}{\textit{Mean Absolute Percentage Error}} \\
    \input{../Output/Tables/covariance_apes_exp.tex} \\[-4mm]
    \hline \\[-1.8ex]
    Simulations & 1,000 & 1,000 & 1,000 & 1,000 & 1,000\\
  \hline
  \hline \\[-1.8ex]
...
Thus, these Monte Carlo simulations tell us that using PCR provides significant performance benefits relative to using a single mismeasured covariate of the true latent covariate in a regression. When the measurements of the true covariate have the same scale and only have Gaussian measurement error, then PCR performs about as well as taking the average of all covariate measurements or as using all measurements as separate covariates in an OLS regression. When we took the exponential of half of the measurements, then PCR outperformed taking the average of the covariate measurements and performed in line with using all measurements as covariates or using an IV technique. These simulations thus tell us that PCR is a useful way of reducing measurement error-caused bias in OLS regression with a latent covariate, and that the performance of this technique is in line with, or better than, other techniques for reducing that bias.
    ...
    \section*{Application: Government Share of Healthcare Spending and Life Expectancy}
        ...
        In Table \ref{main_regs} below we begin with a regression controlling for just a single measurement of GDP per capita PPP, which produces a large and significant coefficient indicating a one percentage point increase in the government share of health expenditure is linked to a increase in life expectancy of 0.194 years.  After this we include a full set of economic covariates listed in the 5 uppermost rows of Table \ref{Sum_Stats}, which reduces the size of the coefficient. However, this result is then adjusted upwards again in the last three columns, where we use the mean of the mismeasured covariates, the first principal component, instrumental variables regression using all the economic indicators as an instrument for GDP per capita (PPP).

        \input{../Output/Tables/LE_Health_Econ_Regressions_wb_only_short.tex}

        The results clearly demonstrate that all of the methods using multiple measures of the covariates produce noticably different coefficients for government health share, relative to the use of single mismeasurement. The usage of many economic indicators directly, the mean, the principal components estimator each produce far smaller coefficients, and the IV estimator produces a slightly smaller coefficient.

        Moreover, these different coefficients behave in a manner similar to that predicted by our theoretical development and simulations. In Table \ref{sim_rho_5_noexp}, we saw the impact of variation in $\rho$, the correlation between measurements for a value of $p = 5$ and $\beta = 1$ for 3,000 observations. In the empirical setting it is difficult to tell what is a reasonable value of $\beta$. Nevertheless, we see that for a positive $\rho$ value between 0 and 1 (as is likely to be the case in light the correlation between GDP and the government share of health spending and overall public goods), the coefficient obtained from using a single measurement is inflated relative to that from PCA, and presumably other methods combining multiple measures as in columns (2), (3), (4), and (5) of Table \ref{main_regs}.

        Results using fixed effects panel models (with country clustered standard errors), and more principal components are in Table \ref{additional_regs}. Fixed effects coefficients greatly reduce the magnitude of any potential causal effects and are insignificant. The results in column (3) also shows a reduction in the inflation of coefficients relative to column (1) of Table \ref{main_regs}. The inclusion of more principal components produces a small coefficient similar to that obtained with just a single component.
...
\clearpage
    ...
    \begin{table}[!htbp] \centering
      ...
      \\[-1.8ex]\hline
      \hline \\[-1.8ex]
      & \multicolumn{3}{c}{\textit{True $\beta_1$}} \
      \cr \
      \\[-1.8ex] & 0.1 & 1 & 10\\
      \hline \\[-1.8ex]
      & \multicolumn{3}{c}{\textit{Coefficient on Main Variable}} \\
      \input{../Output/Tables/beta1_coefs_no_exp.tex}
      & \multicolumn{3}{c}{\textit{Mean Absolute Percentage Error}} \\
      \input{../Output/Tables/beta1_apes_no_exp.tex} \\[-4mm]
      \hline \\[-1.8ex]
      ...
      \hline \\[-1.8ex]
      & \multicolumn{3}{c}{\textit{Coefficient on Main Variable}} \\
        \input{../Output/Tables/beta2_coefs_no_exp.tex}
      & \multicolumn{3}{c}{\textit{Mean Absolute Percentage Error}} \\
       \input{../Output/Tables/beta2_apes_no_exp.tex}  \\[-4mm]
      \hline \\[-1.8ex]
      Simulations & 1,000 & 1,000 & 1,000 \\
      \hline
      \hline \\[-1.8ex]
      ...
      \\[-1.8ex]\hline
      \hline \\[-1.8ex]
      & \multicolumn{3}{c}{\textit{Number of p}} \
      \cr \
      \\[-1.8ex] & 5 & 20 & 50 \\
      \hline \\[-1.8ex]
      & \multicolumn{3}{c}{\textit{Coefficient on Main Variable}} \\
      \input{../Output/Tables/p_coefs_no_exp.tex}
      & \multicolumn{3}{c}{\textit{Mean Absolute Percentage Error}} \\
      \input{../Output/Tables/p_apes_no_exp.tex} \\[-4mm]
      \hline \\[-1.8ex]
      Simulations & 1,000 & 1,000 & 1,000  \\
      \hline
      \hline \\[-1.8ex]
      ...
      \hline \\[-1.8ex]
      & \multicolumn{3}{c}{\textit{Coefficient on Main Variable}} \\
      \input{../Output/Tables/beta1_coefs_exp.tex}
      & \multicolumn{3}{c}{\textit{Mean Absolute Percentage Error}} \\
      \input{../Output/Tables/beta1_apes_exp.tex} \\[-4mm]
      \hline \\[-1.8ex]
       Simulations & 1,000 & 1,000 & 1,000 \\
      \hline
      \hline \\[-1.8ex]
      ...
      \begin{table}[!htbp] \centering
        ...
        \hline \\[-1.8ex]
        & \multicolumn{3}{c}{\textit{Coefficient on Main Variable}} \\
        \input{../Output/Tables/beta2_coefs_exp.tex}
        & \multicolumn{3}{c}{\textit{Mean Absolute Percentage Error}} \\
        \input{../Output/Tables/beta2_apes_exp.tex} \\[-4mm]
        \hline \\[-1.8ex]
        Simulations & 1,000 & 1,000 & 1,000 \\
        \hline
        \hline \\[-1.8ex]
        ...
        \begin{table}[!htbp] \centering
          ...
          \hline \\[-1.8ex]
          & \multicolumn{3}{c}{\textit{Coefficient on Main Variable}} \\
          \input{../Output/Tables/p_coefs_exp.tex}
          & \multicolumn{3}{c}{\textit{Mean Absolute Percentage Error}} \\
          \input{../Output/Tables/p_apes_exp.tex} \\[-4mm]
