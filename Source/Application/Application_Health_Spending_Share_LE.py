...
wb_data = (pd.read_csv(apps_dir + "/WB_Data.csv", index_col=['economy', 'series'])
             ...
             .stack(level = 'economy')
             .rename_axis(None, axis = 1)
             .drop(columns = "SH.XPD.GHED.CH.ZS")
             .rename(columns = {"SP.DYN.LE00.IN":"life_exp", "SH.XPD.CHEX.GD.ZS"
:"gdp_health_share_wb"})
             .rename_axis(['year', 'country'])
             .reset_index()
             .astype({'year': 'int', 'country': 'str'}))
...
# Take mean of world bank and oecd spending percentage or use one or the other if the other is missing
merged_data = (wb_data.sort_index(level=['country', 'year'])
                          .interpolate(limit_area='inside')
                          #.drop(columns=['govt_health_share_wb', 'govt_health_share_oecd'])
                          .dropna()
                          .set_index(['year', 'country']))
...
# Basic time series plot
plt.figure(figsize=(15,15))
plt.plot(std_data.reset_index().set_index('year')['gdp_health_share_wb'])
plt.savefig(figures_dir + "/GDP_Health_Share_Time_Series.pdf")
plt.close()
...
plt.yticks(rotation=0)
plt.xticks(rotation=90)
plt.savefig(figures_dir + "/LE_Health_GDP_Econ_Correlations.pdf")
plt.close()

# OLS for benchmark
ols_benchmark = smf.ols("life_exp ~ gdp_health_share_wb", data = std_data.reset_index()).fit()

print(std_data.reset_index().columns)

# Many covariate OLS
ols_many_covariates = smf.ols("life_exp ~ gdp_health_share_wb + " + covariates_formula_string, data = std_data.reset_index()).fit()

# Panel Fixed Effects Regression for Benchmark
fixed_effects_results = smf.ols("life_exp ~ gdp_health_share_wb + " + covariates_formula_string + " + C(year) + C(country)", data = std_data.reset_index()).fit(cov_type='cluster', cov_kwds={'groups': std_data.reset_index()['country']})

# Decompose into matrix for PCA analysis
# This contains only the economic covariates
X = std_data.drop(columns = ['life_exp', 'gdp_health_share_wb']).to_numpy()
...
# Main PCR spec
partial_pc_regression = smf.ols("life_exp ~ gdp_health_share_wb + PC1", data = std_data).fit()

# PCR with fixed effects
pc_fixed_effects_results = smf.ols("life_exp ~ gdp_health_share_wb + PC1 + C(year) + C(country)", data = std_data.reset_index()).fit(cov_type='cluster', cov_kwds={'groups': std_data.reset_index()['country']})

# Regression table settings
reg_table = Stargazer([ols_benchmark, ols_many_covariates, fixed_effects_results, partial_pc_regression, pc_fixed_effects_results])
reg_table.dependent_variable_name("Life Expectancy at Birth (Years)")
reg_table.covariate_order(['gdp_health_share_wb'])
reg_table.rename_covariates({"gdp_health_share_wb":"GDP Share of Health Exp."})
# Fixed effects indicator
reg_table.add_line('Covariates', ['None', 'Econ Indicators', 'Econ Indicators', 'PCs', 'PCs'])
reg_table.add_line('Fixed Effects', ['No', 'No', 'Yes', 'No', 'Yes'])
reg_table.show_degrees_of_freedom(False)
